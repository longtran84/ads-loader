package vn.fintechviet.loyalty.repository;

import vn.fintechviet.content.ContentExecutionContext;
import vn.fintechviet.loyalty.model.*;
        ...
        return supplyAsync(() -> wrap(em -> {
            try {
                ...
                if (cart.getVoucher() != null) {
                    ...
                    order.setGameCard(cart.getGameCard());
                    pointExchange = cart.getGameCard().getPointExchange();
                } else if (cart.getPhoneCard() != null) {
                    order.setPhoneCard(cart.getPhoneCard());
                ...
                em.remove(cart);
                User user = getUserByDeviceToken(deviceToken);
                user.setEarning(user.getEarning() - pointExchange * quantity);
                em.merge(user);
                em.flush();
                return "ok";
            } catch (Exception ex) {
